...
def parse(tokens: List[odes.Token]):
    ...
    for token in tokens:
        ...
        # number
            ...
            if inversed:
                ouqueue.append(odes.Token('0', 'int'))
                inversed = False
...
def check(a, b):
    ...
    for tok in a:
        s += str(tok) + ' '
    assert s[:-1]==b, 'Error: ' + s + ' != ' + b

def main():
    # print(parse(odes.tokenize('t+5 = 3*sin(2*pi*t)+int(t, 15)')))
    check(parse(odes.tokenize('a + b')), 'a b +')
    check(parse(odes.tokenize('-a + b')), '0 a - b +')
    check(parse(odes.tokenize('a + -b')), 'a 0 b - +')
    check(parse(odes.tokenize('a - b')), 'a b -')
    check(parse(odes.tokenize('-a - b')), '0 a - b -')
    check(parse(odes.tokenize('a - -b')), 'a 0 b - -')
    check(parse(odes.tokenize('-a - -b')), '0 a - 0 b - -')
